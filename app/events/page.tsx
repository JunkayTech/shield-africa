        ...
        <div className="mx-auto max-w-3xl px-5 sm:px-8">
          <Reveal>
            <div className="overflow-hidden rounded-3xl border border-border bg-surface-elevated p-8">
              <h2 className="font-serif text-2xl font-medium">Registration Details</h2>
...
/* ---------------------------
   ...
   --------------------------- */
function RegistrationForm({ upcomingEvents }: { upcomingEvents: { slug: string; title: string; date: string }[] }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!form.consent) {
      setError("Consent is required to receive updates from Shield Africa.");
      return;
    }
    ...
    try {
      ...
      // Example: await fetch("/api/register", { method: "POST", body: JSON.stringify(payload) });

      setSubmitting(false);
      alert("Registration submitted. We will contact you with next steps.");
      // Reset form if desired
      setForm((s) => ({ ...s, firstName: "", lastName: "", email: "", phone: "", organization: "", jobTitle: "", sector: "", sectorOther: "", state: "", stateOther: "" }));
    } catch (err) {
      console.error(err);
  ...
  return (
    <form onSubmit={handleSubmit} className="mt-6 grid gap-4">
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
        <label className="flex flex-col">
          <span className="text-sm font-medium">Organization / Institution</span>
          <input
        ...
        <label className="flex flex-col">
          <span className="text-sm font-medium">Job Title / Role</span>
          <input
            value={form.jobTitle}
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
        <label className="flex flex-col">
          <span className="text-sm font-medium">Sector / Industry</span>
          <select
            value={form.sector}
        ...
        {form.sector === "Other" && (
          <label className="flex flex-col">
            <span className="text-sm font-medium">Please specify sector</span>
            <input
      ...
      {form.state === "Other" && (
        <label className="flex flex-col">
          <span className="text-sm font-medium">Please specify state</span>
          <input
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 items-center">
        <div>
          <div className="text-sm font-medium">Would you like to receive information about future Shield Africa programmes?</div>
          <div className="mt-2 flex gap-3">
            <label className={`inline-flex items-center gap-2 rounded-full border px-3 py-2 ${form.futureInfo === "yes" ? "bg-foreground text-background" : "bg-surface-elevated text-muted-foreground"}`}>
              <input type="radio" name="futureInfo" value="yes" checked={form.futureInfo === "yes"} onChange={() => update("futureInfo", "yes")} />
              Yes
            </label>
            <label className={`inline-flex items-center gap-2 rounded-full border px-3 py-2 ${form.futureInfo === "no" ? "bg-foreground text-background" : "bg-surface-elevated text-muted-foreground"}`}>
              <input type="radio" name="futureInfo" value="no" checked={form.futureInfo === "no"} onChange={() => update("futureInfo", "no")} />
              No
            </label>
          </div>
        </div>
        ...
        <label className="flex items-start gap-3">
          ...
          <div>
            <div className="text-sm font-medium">Consent to receive updates from Shield Africa</div>
            <div className="text-sm text-muted-foreground">I consent to receive updates and communications from Shield Africa.</div>
          </div>
        </label>
      </div>
      ...
      <div className="flex items-center gap-3">
        ...
        <span className="text-sm text-muted-foreground">We will contact you with next steps.</span>
      </div>
    </form>
  );
}
